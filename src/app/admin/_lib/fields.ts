...
export const TABLE_CONFIGS: Record<string, TableConfig> = {
  products: {
    ...
    fields: [
      ...
      { column: "net_weight", label: "규격 — 내용량", type: "text", help: "비워 두면 화면에 '문의 시 안내'로 표시됩니다. 예: 1kg" },
      { column: "box_qty", label: "규격 — 박스 입수", type: "text", help: "예: 10팩/박스" },
      { column: "storage", label: "규격 — 보관 방법", type: "text", help: "예: -18℃ 이하 냉동 보관" },
      { column: "shelf_life", label: "규격 — 유통기한", type: "text", help: "예: 제조일로부터 12개월" },
      { column: "ingredients", label: "규격 — 원재료", type: "text" },
      { column: "origin", label: "규격 — 원산지", type: "text", help: "예: 돼지고기(국내산)" },
      { column: "hs_code", label: "규격 — HS 코드", type: "text", help: "관세사 확인 후 입력" },
      { column: "spec_sheet_url", label: "규격 — 스펙시트 PDF 경로", type: "text", help: "/downloads/… (public 폴더 기준)" },
...
export const SETTING_FIELDS: { key: string; label: string; type: "text" | "textarea" }[] = [
  ...
  { key: "hero_descriptor", label: "Hero 아래 설명형 한 줄 (무엇을 하는 회사인지)", type: "textarea" },
  { key: "hero_alt", label: "Hero 이미지 대체 텍스트(ALT)", type: "text" },
  { key: "hero_media", label: "Hero 미디어 경로 (.jpg 또는 .mp4)", type: "text" },
  { key: "stat_founded", label: "통계 — 창립연도", type: "text" },
  { key: "stat_revenue", label: "통계 — 연 매출", type: "text" },
