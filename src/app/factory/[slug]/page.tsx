...
import { getFactory, listFactories, listProducts } from "@/lib/db/repo";
...
export default async function FactoryDetailPage({ params }: Props) {
  ...
  const products = listProducts().filter((p) => p.factorySlug === factory.slug).slice(0, 3);
  const others = listFactories().filter((f) => f.slug !== factory.slug);
    ...
    <>
      ...
      <section className="section">
        <div className="container-text">
          <SectionTitle kicker="Role" title={factory.role} />
          <Reveal>
      ...
      <section className="section hairline-t bg-paper-warm">
        <div className="container-text">
          <SectionTitle kicker="Capabilities" title="이 공장이 하는 일" />
      ...
      {products.length > 0 && (
        <section className="section hairline-t">
          <div className="container-grid">
            <SectionTitle kicker="Made Here" title={`${factory.name}에서 만드는 제품`} />
      ...
      <section className="section hairline-t bg-paper-warm">
        <div className="container-grid">
          <SectionTitle kicker="Other Factories" title="다른 공장 보기" />
          <div className="grid gap-6 sm:grid-cols-2">
            {others.map((other) => (
              <Link
                key={other.slug}
                href={`/factory/${other.slug}`}
